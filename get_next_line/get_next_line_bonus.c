...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/21 23:59:59 by kkeskin           #+#    #+#             */
/*   Updated: 2025/08/21 23:59:59 by kkeskin          ###   ########.tr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		text = ft_strdup(keep[fd]);
		if (!text)
			return (free(keep[fd]), NULL);
	}
	if (keep[fd] && !*text && BUFFER_SIZE > 2)
		return (free(keep[fd]), free(text), NULL);
	if (!ft_readline(fd, &newline, &text, &keep[fd]))
		return (free(text), NULL);
	if (!ft_splitfromnew(&text, newline, &keep[fd]))
		return (free(keep[fd]), NULL);
	return (text);
}
